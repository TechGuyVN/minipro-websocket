...
static void responseHandler(switch_core_session_t *session, const char *eventName, char *json)
{
	...
	if (json)
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "responseHandler: %s sending event payload: %s.\n", eventName, json);
...
/*
* This function use to capture callback from Bug assign to session
* when call processing, RTP package will be bug to frame data, we can use to fork to ASR module
*/
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
	...
	case SWITCH_ABC_TYPE_READ:
		// return fork_frame_replace(session, bug);
		// switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Got SWITCH_ABC_TYPE_READ.\n");
		break;
...
/*
* this func to setting session like assign bug, fork init audio , start websocket to kaldi server ...
*/
									 ...
									 const char *base)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
					  "mod_audio_fork: streaming %d sampling to %s\n",
					  sampling, host);
	...
	if (SWITCH_STATUS_FALSE == fork_session_init(session, responseHandler, read_codec->implementation->actual_samples_per_second,
												 host, sampling, channels, metadata, &pUserData))
	{
	...
	else
	{
		...
		if ((lsession = switch_core_session_locate(argv[0])))
		{
			...
			else if (!strcasecmp(argv[1], "start"))
			{
				...
				{
					strcpy(host,argv[2]);
					status = start_capture(lsession, flags, host, sampling, metadata, "mod_audio_fork");
				}
			}
			else
			{
